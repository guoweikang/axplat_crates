...
fn main() {
    ...
    match arch.as_str() {
        ...
        "x86_64" => {
            use_linker_script_x86_64();
        }
        "riscv64" => {
            use_linker_script_riscv64();
        }
        _ => {
...
fn generate_linker_script_aarch64(out_dir: &str) {
    ...
    let linker_script_path = PathBuf::from(&manifest_dir).join("linker-aarch64.lds");

    println!("cargo:rerun-if-changed={}", linker_script_path.display());

    // Read template
    let ld_content = fs::read_to_string(&linker_script_path).unwrap_or_else(|e| {
        panic!(
    ...
    fs::write(&dst, ld_content).unwrap_or_else(|e| panic!("Failed to write linker script:  {}", e));
...
fn use_linker_script_x86_64() {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set");

    let linker_script_path = PathBuf::from(&manifest_dir).join("linker-x86_64.lds");

    println!("cargo:rerun-if-changed={}", linker_script_path.display());
    println!("cargo:rustc-link-arg=-T{}", linker_script_path.display());
    println!(
        "cargo:warning=Using x86_64 linker script: {}",
        linker_script_path.display()
    );
}

fn use_linker_script_riscv64() {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR not set");

    let linker_script_path = PathBuf::from(&manifest_dir).join("linker-riscv64.lds");

    println!("cargo:rerun-if-changed={}", linker_script_path.display());
    println!("cargo:rustc-link-arg=-T{}", linker_script_path.display());
    println!(
        "cargo:warning=Using riscv64 linker script: {}",
        linker_script_path.display()
    );
}
